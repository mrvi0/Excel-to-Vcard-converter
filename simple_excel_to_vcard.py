#!/usr/bin/env python3
"""
Simple Excel to vCard Converter

A simplified version that converts only basic contact fields:
- Name
- Surname  
- Phone
- Email (optional)
...
class SimpleExcelToVcardConverter:
    """Simple converter for basic contact fields."""
    ...
    def create_simple_vcard_entry(self, row: pd.Series) -> str:
        """
        Create a simple vCard entry with basic fields.
        ...
        # Name field (N:LastName;FirstName;;;)
        vcard += f"N:{surname};{name};;;\n"
    ...
    def convert(self) -> bool:
        ...
        try:
            ...
            for index, row in self.data.iterrows():
                vcard_entry = self.create_simple_vcard_entry(row)
                if vcard_entry:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert Excel file to vCard format (simple version)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python simple_excel_to_vcard.py Contacts.xlsx
  python simple_excel_to_vcard.py data.xlsx --sheet "Contacts" --output "simple_contacts.vcf"
        """
    )
